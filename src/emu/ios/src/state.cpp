...
namespace eka2l1::ios {
    ...
    void emulator::stage_one() {
        ...
        LOG_INFO(FRONTEND_CMDLINE, "EKA2L1 iOS frontend ({}-{})", GIT_BRANCH, GIT_COMMIT_HASH);
    ...
    bool emulator::stage_two() {
        ...
        if (!dvc) {
            LOG_ERROR(FRONTEND_CMDLINE, "No current device is available. Copy an EKA2L1 data folder into the app documents directory.");
            return false;
        }
