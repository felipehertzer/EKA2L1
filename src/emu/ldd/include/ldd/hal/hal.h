...
namespace eka2l1::ldd {
    static constexpr const char *HAL_FACTORY_NAME = "RHalDriver";

    enum hal_control_opcode {
        hal_control_opcode_set_value = 6,
        hal_control_opcode_get_value = 7
    };

    class hal_channel : public channel {
    public:
        explicit hal_channel(kernel_system *kern, system *sys, epoc::version ver);
        ~hal_channel() override {}
    ...
    class hal_factory : public factory {
    public:
        explicit hal_factory(kernel_system *kern, system *sys);

        ~hal_factory() override {
        }
