...
namespace eka2l1::vita {
    ...
    static void on_vita_touch(void *userdata, vec3 pos, int button, int action, int pointer_id) {
        (void)button;
        emulator *state = reinterpret_cast<emulator *>(userdata);
        if (state) {
            touch_screen(*state, pos.x, pos.y, pos.z, action, pointer_id);
        }
    }

    static void on_vita_key_press(void *userdata, std::uint32_t key) {
        emulator *state = reinterpret_cast<emulator *>(userdata);
        if (state) {
            press_key(*state, static_cast<int>(key), static_cast<int>(drivers::key_state::pressed));
        }
    }

    static void on_vita_key_release(void *userdata, std::uint32_t key) {
        emulator *state = reinterpret_cast<emulator *>(userdata);
        if (state) {
            press_key(*state, static_cast<int>(key), static_cast<int>(drivers::key_state::released));
        }
    }
    ...
    static int graphics_driver_thread_initialization(emulator &state) {
        ...
        state.window = std::make_unique<drivers::emu_window_vita>();
        state.window->raw_mouse_event = on_vita_touch;
        state.window->button_pressed = on_vita_key_press;
        state.window->button_released = on_vita_key_release;
        state.window->init("EKA2L1", eka2l1::vec2(960, 544), drivers::emu_window_flag_maximum_size);
        state.window->set_userdata(&state);

        state.graphics_driver = drivers::create_graphics_driver(drivers::graphic_api::native,
        ...
        drivers::emu_window_vita *window = state.window.get();
