...
namespace eka2l1::epoc::drm {
    ...
    bool rights_database::add_or_update_record(rights_object &obj) {
        ...
        for (std::size_t i = 0; i < obj.permissions_.size(); i++) {
            ...
            sqlite3_bind_int(add_perm_info_stmt_, 3, static_cast<int>((static_cast<std::uint32_t>(obj.permissions_[i].version_rights_main_) << 16) | obj.permissions_[i].version_rights_sub_));
    ...
    int rights_database::count_records() {
        sqlite3_stmt *stmt = nullptr;
        static const char *COUNT_RECORDS_STMT_STRING = "SELECT COUNT(*) FROM RightInfo";

        if (sqlite3_prepare(database_, COUNT_RECORDS_STMT_STRING, -1, &stmt, nullptr) != SQLITE_OK) {
            LOG_ERROR(SERVICE_DRMSYS, "Fail to prepare rights record count statement!");
            return 0;
        }
