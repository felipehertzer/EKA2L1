...
namespace eka2l1 {
    ...
    private:
        std::unique_ptr<service::ipc_context> file_lock_event_msg_;
        std::unique_ptr<service::ipc_context> backup_operation_event_msg_;
        bool backup_operation_observer_present_ = false;

    public:
        explicit backup_old_session(service::typical_server *serv, const kernel::uid ss_id, epoc::version client_version);
        ~backup_old_session() override;
