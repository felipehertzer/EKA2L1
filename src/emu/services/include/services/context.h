...
namespace eka2l1 {
    ...
    namespace service {
        ...
        struct ipc_context {
            ...
            /**
             * \brief    Write a server-side local handle to an IPC argument.
             *
             * Symbian clients may pass the return slot either as a descriptor-backed
             * package or as a handle/integer argument, depending on the client-side
             * wrapper used for that subsession.
             *
             * \param    idx  The index of the IPC argument, in range of [0, 3].
             * \param    data The handle value.
             *
             * \returns  True on success.
             */
            bool write_handle_argument(int idx, uint32_t data);
