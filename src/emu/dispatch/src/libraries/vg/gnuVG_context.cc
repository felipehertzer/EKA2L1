...
namespace gnuVG {
    ...
    void Context::destroy(eka2l1::drivers::graphics_driver *driver, eka2l1::drivers::graphics_command_builder &builder) {
        ...
        for (const auto &obj : cleanup_lists) {
    ...
    void Context::flush_to_driver(eka2l1::dispatch::egl_controller &controller, eka2l1::drivers::graphics_driver *driver, const bool is_frame_swap_flush) {
        ...
        eka2l1::epoc::canvas_base *auto_present_window = nullptr;
        if (!is_frame_swap_flush && controller.should_auto_present(this)) {
            auto_present_window = controller.queue_present_context_surface(this, driver);
        }
    ...
    void Context::vgMultMatrix(const VGfloat *m) {
        ...
        Matrix result;
        result.multiply(matrix[user_matrix], b);
    ...
    void Context::vgSetf(VGint paramType, VGfloat value) {
        ...
        case VG_STROKE_MITER_LIMIT:
            miter_limit = value;
            if (miter_limit < 1.0f)
                miter_limit = 1.0f; // silently clamp small/negative values to 1.0f
            break;
...
namespace gnuVG {
    ...
    void Context::trivial_render_framebuffer(const GraphicState &state,
        ...
        auto f = [this, vertices, vertices_full, mat, mat_full, indices, texture_coords, wrap_mode, state, do_glyph](const FrameBuffer *fbuffer,
                     bool do_horizontal_gauss,
                     int kern_diameter) {
            int caps = (do_glyph ? Shader::do_texture : Shader::do_pattern);
            if (kern_diameter > 1) {
                caps |= do_horizontal_gauss ? Shader::do_horizontal_gaussian : Shader::do_vertical_gaussian;
                caps |= (kern_diameter << Shader::gauss_krn_diameter_shift) & Shader::gauss_krn_diameter_mask;
            }
            ...
            if (do_glyph) {
                ...
                active_shader->set_pattern_matrix(do_horizontal_gauss ? mat_full : image_matrix_data);
    ...
    void Context::trivial_render_elements(const GraphicState &state,
        ...
        active_shader = state.shaderman->get_shader(state.driver,
            Shader::do_flat_color | (do_color_transform ? Shader::do_color_transform : 0));
        active_shader->set_current_graphics_command_builder(cmd_builder_);
        active_shader->use_shader();
        active_shader->set_matrix(mat);
    ...
    void Context::select_conversion_matrix(MatrixMode _conversion_matrix) {
        if (conversion_matrix != _conversion_matrix || matrix_is_dirty[_conversion_matrix] || matrix_is_dirty[GNUVG_MATRIX_FILL_PAINT_TO_USER] || matrix_is_dirty[GNUVG_MATRIX_STROKE_PAINT_TO_USER]) {
    ...
    void Context::use_pipeline(const GraphicState &state, gnuVGPipeline new_pipeline, VGPaintMode _mode) {
        ...
        auto active_paint = pipeline_mode == VG_STROKE_PATH ? stroke_paint : fill_paint;
        ...
        if (mask_is_active)
            active_shader->set_mask_texture(mask.color_buffer);
    ...
    void Context::render_to_framebuffer(const GraphicState &state, FrameBuffer *framebuffer) {
        ...
        if (current_framebuffer->width == buffer_width && current_framebuffer->height == buffer_height)
