    ...
    void Paint::vgSetParameterfv(VGint paramType, VGint count, const VGfloat *values) {
        ...
        case VG_PAINT_COLOR_RAMP_STOPS:
            if (count > 0) {
                ...
                max_stops = new_stops > GNUVG_MAX_COLOR_RAMP_STOPS ? GNUVG_MAX_COLOR_RAMP_STOPS : new_stops;
                ...
                for (;
                    ...
                    if (k + 1 < max_stops)
                        color_ramp_stop_invfactor[k] = 1.0f / (values[v_off + 5] - values[v_off]);
                    else
        ...
        case VG_PAINT_RADIAL_GRADIENT:
            if (count == 5) {
                 ...
                 * 1 / (r^2 − (fx'^2 + fy'^2))
                 */
                gradient_parameters[5] = 1.0f / (gradient_parameters[4] - (gradient_parameters[2] * gradient_parameters[2] + gradient_parameters[3] * gradient_parameters[3]));
                return;
            }
            break;
        }
...
namespace eka2l1::dispatch {
    BRIDGE_FUNC_LIBRARY(VGPaint, vg_create_paint_emu) {
        ...
        if (context) {
            ...
            if (paint)
                return (VGPaint)paint->get_handle();
        }
