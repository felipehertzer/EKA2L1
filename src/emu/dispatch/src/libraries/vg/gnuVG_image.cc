...
namespace gnuVG {
    ...
    VGImage Image::vgChildImage(VGint x, VGint y, VGint width, VGint height) {
        ...
        while (beg != end) {
            ...
            if ((child->subset_info.original_subset_x == x) && (child->subset_info.original_subset_y == y) && (child->subset_info.subset_width == width) && (child->subset_info.subset_height == height)) {
